...
def LastPages(pdf,data,config,page_num,timeline_df,time_range,error_lines,data_manager):
  ...
  for pg in range(config['timeline']['npages']):
    ...
    with AddEmptyPage(pdf,page_num,config) as page:
      ...
      posy = 0.930 - size
      page.ax1 = page.fig.add_axes((0.100,posy, 0.607,size), zorder=4)
      page.ax1.set_title("Timeline", fontweight='bold')
      # Configuring axes ticks, ranges and labels
      page.ax1.yaxis.set_minor_locator(AutoMinorLocator(2))
      page.ax1.yaxis.grid(color='gray', linestyle='-', alpha=0.4, which='minor')
      page.ax1.tick_params(which='minor', left=False)
      page.ax1.set_ylabel('Step')
      page.ax1.set_ylim((nsteps_this_page-0.5,-0.5))
      page.ax1.set_yticks(range(nsteps_this_page))
      # page.ax1.set_yticklabels(timeline_df_this_page['step'])
      page.ax1.tick_params(top=False, bottom=True, left=False, right=False, labelleft=True, labelbottom=True)
      ...
      for index, row in timeline_df_this_page.iterrows():
        ...
        # page.ax1.axhspan(index-0.5,index+0.5, color=['black','white'][index%2], alpha=0.1, zorder=0)
        # Adding state 'st' as text at the right of the bar  
        page.ax1.text(0.99, index-last_step_prev_page-1+0.05, row['name'], color='black', ha='right', va='center',fontsize=(config['appearance']['smallfont'] if nsteps_this_page<=90 else config['appearance']['tinyfont'] ), transform=page.ax1.get_yaxis_transform())
        page.ax1.text(1.01, index-last_step_prev_page-1+0.05, row['st'].replace("CANCELLED by","CANCELLED\nby"), color=row['edgecolor'], ha='left', va='center',fontsize=(config['appearance']['normalfont'] if nsteps_this_page<=90 else config['appearance']['smallfont'] ), fontweight='bold', transform=page.ax1.get_yaxis_transform())
        # if (row['rc'] != 0) or (row['sig'] != 0):
        page.ax1.text(0.94, index-last_step_prev_page-1+0.05, f"Return Code: {row['rc']}, Signal: {row['sig']}", ha='right', va='center', fontsize=config['appearance']['tinyfont'], color='k', transform=transforms.blended_transform_factory(page.fig.transFigure, page.ax1.transData)) 
      page.ax1.barh(timeline_df_this_page['step'], timeline_df_this_page['duration'], left=timeline_df_this_page['start_time'], color=timeline_df_this_page['color'], edgecolor=timeline_df_this_page['edgecolor'], linewidth=0.5)
      posy -= 0.030
