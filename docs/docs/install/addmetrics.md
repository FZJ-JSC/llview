...
1. The first step is to create a plugin that will read the metrics from its source and generate an LML file with the desired quantities. One of the existing plugins in the folder `${LLVIEW_HOME}/da/rms/` (e.g., Slurm, Generic API, Log Files, or Git) can be used as examples or as a starting point.
